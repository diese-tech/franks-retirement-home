...
// A rate-limit identifier is persisted as literal Redis key material (visible
// to anyone with Redis observability/read access) and can also show up in
// application logs. Callers that key on a real credential -- a shared
// captain/admin draft key, not an opaque id like a Discord user id -- must
// hash it first so the stored identifier can't be used to reconstruct or
// replay the credential itself.
