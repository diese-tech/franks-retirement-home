...
// after SESSION_TTL_MS. This is intentionally simple — no separate session
// store, no rotating IDs. It is sufficient for a single-admin dashboard and
// mirrors the security guarantees of the current shared password.
//
// Behavior is gated by ADMIN_AUTH_REQUIRED so the cookie path can ship
// disabled and be flipped on after the prod deployment is verified.
...
/**
 * Whether admin auth is enforced for this deployment.
 * Defaults to false to allow safe rollout; set ADMIN_AUTH_REQUIRED=true to enforce.
 */
 ...
 *
 * Returns a NextResponse 401 when auth is required and the cookie is missing
 * or invalid. Returns null when auth is not required (rollout flag off) OR
