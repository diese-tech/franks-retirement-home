...
export async function getEffectiveVaultedGodIds(draftId) {
  ...
  // Only PICKS from prior games are vaulted — bans do not carry over between
  // games in a series (see docs/draft-architecture.md §2 Vault Scope Contract).
    ...
    select: {
      draft: {
        select: {
          ...
          // bans intentionally omitted — cross-game ban vaulting is not a rule
        },
      },
    },
  });
  ...
  for (const game of siblingGames) {
    ...
    // Bans are NOT added here. Current-draft bans still block picking within
    // the same draft (checked separately in the pick route), but they must
    // not vault gods into later games of the same series.
  }
