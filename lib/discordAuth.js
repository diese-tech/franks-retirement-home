...
/**
 * Reads the Discord session from a Next.js cookie store (server component context)
 * and checks if the user has admin role.
 *
 * Use this in server components instead of constructing a synthetic request object.
 *
 * @param {import('next/headers').ReadonlyRequestCookies} cookieStore - from cookies()
 * @returns {{ isAdmin: boolean, session: object|null }}
 */
export function getAdminStatusFromCookies(cookieStore) {
  try {
    const sessionCookie = cookieStore.get(DISCORD_SESSION_COOKIE);
    if (!sessionCookie?.value) return { isAdmin: false, session: null };

    // Build a minimal request-like object for getDiscordSessionUser
    const fakeReq = {
      headers: {
        get: (name) => {
