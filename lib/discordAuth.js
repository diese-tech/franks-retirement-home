...
/**
 * Checks if roles include DISCORD_CAPTAIN_ROLE_ID.
 * @param {string[]} roles
 * @returns {boolean}
 */
export function hasDiscordCaptainRole(roles) {
  const captainRoleId = process.env.DISCORD_CAPTAIN_ROLE_ID;
  if (!captainRoleId) return false;
  return roles.includes(captainRoleId);
}
...
/**
 * Given roles array, checks division role membership.
 * @param {string[]} roles
 * @returns {'div-s9-hospice' | 'div-s9-rehabilitation' | null}
 */
export function resolveDivisionFromRoles(roles) {
  const hospiceRoleId = process.env.DISCORD_HOSPICE_ROLE_ID;
  const rehabRoleId = process.env.DISCORD_REHABILITATION_ROLE_ID;

  if (hospiceRoleId && roles.includes(hospiceRoleId)) return 'div-s9-hospice';
  if (rehabRoleId && roles.includes(rehabRoleId)) return 'div-s9-rehabilitation';
  return null;
}
