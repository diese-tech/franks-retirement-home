...
| Area | Score | Why |
|---|---|---|
| **Production readiness** | **6/10** | Solid bones (CI correctly gates migrations to main-push only, Sentry is genuinely wired for client-side errors, a real `/api/health` endpoint exists, zero abandoned TODO/stub code in `app/`/`lib/`, rate limiting exists and works where it's applied) undercut by a real, long-documented security gap (RLS disabled on all 38 pre-existing tables) and inconsistent defense-in-depth (rate limiting covers 9 of ~80 route files; a few routes have zero error handling and would 500 uncleanly; API-layer errors never reach Sentry, only client-side ones do; e2e tests never run in CI). |
...
| **Captain/player usability** | **6/10** | Match reporting, drafts, and tournaments all work well for captains. But `PlayerDraft` picks are still admin-only (a captain can't self-serve their own pick, a known deferred gap), the client has no clean "am I a captain" signal (`/api/auth/discord/me` only exposes `isAdmin`/`teamId`), and `/captain` has no gate of its own — a non-captain hitting it directly just sees a confusing empty dashboard rather than a clear message. |
...
1. **RLS is disabled on all 38 pre-existing `public` tables** (confirmed still accurate as of HEAD — the only RLS-enabling migration is the tournament feature's, which covers just its own 3 new tables). This is real: Supabase's PostgREST Data API is reachable independently of the app's own Prisma-layer checks. Low-risk to fix — Prisma's `postgres` role already has `BYPASSRLS`, so enabling RLS with no policies (the same pattern already applied to the 3 tournament tables) closes the gap with zero effect on app behavior. Documented in `docs/adr/0005`, unresolved since.
...
1. **PlayerDraft picks are still admin-only** — a captain cannot make their own pick; an admin has to do it for them. This was already a known, deliberately deferred gap (`docs/season-9-ops-reference.md`), not new, but it's the single biggest captain-facing usability gap in the app.
...
- **`docs/adr/0005`** needs a short follow-up confirming the tournament tables' RLS landed (it did) and that the other 38 tables' gap is still open (it is, until this pass's plan closes it).
