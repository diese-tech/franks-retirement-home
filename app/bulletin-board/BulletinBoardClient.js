...
const TYPE_META = {
  ...
  match_hype:       { icon: '⚔️', label: 'Match Hype',        desc: 'Pre-game breakdowns and predictions' },
...
function PostCard({ post }) {
  return (
    <div style={{
      borderBottom: '1px solid #14141420',
      padding: '12px 14px',
      display: 'flex',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        ...
        {post.pinned && (
          <span style={{ fontFamily: 'VT323, monospace', fontSize: 11, color: '#CC3300' }}>
            📌 PINNED
          </span>
        )}
      </div>
      <div style={{ fontFamily: 'Boogaloo, cursive', fontSize: 17, letterSpacing: '0.03em' }}>
        {post.title}
      </div>
      {post.excerpt && (
...
function CategoryRow({ type, count, lastPost }) {
  ...
  return (
    <div className="bb-cat">
      ...
      <div className="bb-cat__last">
        {lastPost ? formatDate(lastPost.publishedAt ?? lastPost.createdAt) : '—'}
      </div>
    </div>
  );
}

export default function BulletinBoardClient({ posts, totalCount }) {
  const dbError = posts === null;
  const publishedPosts = posts ?? [];

  const countByType = ALL_TYPES.reduce((acc, t) => {
    acc[t] = publishedPosts.filter(p => p.type === t).length;
    return acc;
  }, {});

  const lastByType = ALL_TYPES.reduce((acc, t) => {
    const typed = publishedPosts.filter(p => p.type === t);
    acc[t] = typed[0] ?? null;
    return acc;
  }, {});
    ...
    <div>
      ...
      <div className="frh-page-masthead">
        ...
        <div className="frh-page-masthead__sub">
          FRH Community &middot; League announcements &middot; Player spotlights
        </div>
        <div className="frh-page-masthead__stats">
          <div className="frh-statchip">
            <span className="frh-statchip__val">{publishedPosts.length}</span>
            <span className="frh-statchip__lbl">Posts</span>
          </div>
          <div className="frh-statchip">
            <span className="frh-statchip__val">{totalCount}</span>
            <span className="frh-statchip__lbl">All Time</span>
          </div>
          <div className="frh-statchip">
            <span className="frh-statchip__val">{ALL_TYPES.length}</span>
            <span className="frh-statchip__lbl">Categories</span>
          </div>
        </div>
      </div>
                ...
                <div className="thread" style={{ marginBottom: 16 }}>
                  ...
                  <div className="thread__post">
                    <div className="thread__author">
                      ...
                      <div className="thread__author__name">
                        {featuredPost.createdById ?? 'FRH Staff'}
                      </div>
                      <div className="thread__author__meta">
                    ...
                    <div className="thread__body">
                      ...
                      {featuredPost.relatedTeam && (
                        <div className="thread__sig">
                          Re: {featuredPost.relatedTeam.name}
                          {featuredPost.relatedPlayer && ` · ${featuredPost.relatedPlayer.name}`}
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="frh-panel">
                  <div className="frh-panel__body" style={{ textAlign: 'center', padding: '48px 20px' }}>
                    ...
                    <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 10, opacity: 0.6, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                      No published posts. Check back soon.
                    </div>
                  </div>
                </div>
              )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div className="frh-panel">
                ...
                <div className="frh-panel__body" style={{ padding: '10px 12px' }}>
                  {[
                    'Keep it in the spirit of the league',
                    'No doxxing or real-life threats',
                    'Save the salt for post-game',
                    'Admins have final say on all rulings',
                    'Fraud Watch tips go in the right section',
                  ].map((rule, i) => (
                    <div key={i} style={{
                      fontFamily: 'Share Tech Mono, monospace',
                      fontSize: 10,
                      ...
                      borderBottom: '1px dotted #14141420',
                      position: 'relative',
                    }}>
                      <span style={{ position: 'absolute', left: 0, color: '#CC3300' }}>▸</span>
                      {rule}
                    </div>
                  ))}
                </div>
              ...
              {/* Quick stats */}
              <div className="frh-panel">
                <header className="frh-panel__titlebar frh-panel__titlebar--lime">
                  <div className="frh-panel__ttl">
                    <span className="frh-panel__accent" />
                    Board Stats
                  </div>
                  ...
                  {ALL_TYPES.map(type => {
                    ...
                    return (
                      <div key={type} style={{
                        ...
                        fontSize: 10,
                        padding: '4px 0',
                        borderBottom: '1px solid #14141410',
                      }}>
                        <span style={{ opacity: 0.7 }}>{meta.icon} {meta.label}</span>
                        <span style={{ fontFamily: 'Boogaloo, cursive', fontSize: 14, color: '#1040aa' }}>
                          {countByType[type]}
                        </span>
                      </div>
                    );
