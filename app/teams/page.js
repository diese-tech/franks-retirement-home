      ...
      <RetroWindow title="TEAMS.EXE" titleBarColor="yellow">
        <div className="flex items-start justify-between gap-4 mb-6 border-b-2 border-brand-700 pb-4">
          <div>
            ...
            {activeSeason && (
              <p className="text-sm text-gray-500">{activeSeason.name} — {teams.length} team{teams.length !== 1 ? 's' : ''}</p>
            )}
          </div>
          {activeSeason && (
        ...
        {teams.length === 0 ? (
          <p className="text-sm text-gray-600 text-center py-8">No teams yet. Check back once the season kicks off.</p>
        ) : (
          Object.entries(byDivision).sort(([a], [b]) => a.localeCompare(b)).map(([divName, divTeams]) => (
            <div key={divName} className="mb-8">
              <div className="flex items-center gap-2 mb-3">
                <span className="font-ui text-xs uppercase tracking-widest text-gray-500">Division</span>
                <PixelBadge label={divName} color={DIVISION_COLOR[divName] ?? 'gray'} />
                <span className="text-[10px] text-gray-600">{divTeams.length} team{divTeams.length !== 1 ? 's' : ''}</span>
              </div>
                ...
                {divTeams.map((team) => {
                  ...
                  return (
                    <Link key={team.id} href={`/teams/${team.id}`} className="block group">
                      <div className="border-2 border-brand-700 group-hover:border-frh-yellow/50 transition-colors p-3 bg-brand-950/40">
                        <div className="flex items-center justify-between mb-2">
                          <div>
                            <span className="font-ui text-sm text-gray-200 group-hover:text-frh-yellow transition-colors">{team.name}</span>
                            <span className="ml-2 font-mono text-[10px] text-gray-600 border border-brand-600 px-1">[{team.tag}]</span>
                          </div>
                          <span className="text-[10px] text-gray-600">{team.members.length} players</span>
                        </div>
                        {captain && (
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {team.members.slice(0, 5).map((m) => (
                            <span key={m.id} className={`text-[9px] font-mono ${ROLE_COLORS[m.player.role] ?? 'text-gray-500'}`}>
                              {m.player.role[0]}
                            </span>
                          ))}
                        </div>
